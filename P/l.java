package p;

public final class l implements Cloneable {
    public boolean i;
    public int[] j;
    public Object[] k;
    public int l;
    public static final Object m;

    static {
        l.m = new Object();
    }

    public l() {
        this(10);
    }

    public l(int v) {
        this.i = false;
        if(v == 0) {
            this.j = d.a;
            this.k = d.b;
            return;
        }
        int v1 = v * 4;
        for(int v2 = 4; v2 < 0x20; ++v2) {
            int v3 = (1 << v2) - 12;
            if(v1 <= v3) {
                v1 = v3;
                break;
            }
        }
        this.j = new int[v1 / 4];
        this.k = new Object[v1 / 4];
    }

    public final void a(int v, Object object0) {
        int v1 = this.l;
        if(v1 != 0 && v <= this.j[v1 - 1]) {
            this.e(v, object0);
            return;
        }
        if(this.i && v1 >= this.j.length) {
            this.c();
        }
        int v2 = this.l;
        if(v2 >= this.j.length) {
            int v3 = (v2 + 1) * 4;
            for(int v4 = 4; v4 < 0x20; ++v4) {
                int v5 = (1 << v4) - 12;
                if(v3 <= v5) {
                    v3 = v5;
                    break;
                }
            }
            int[] arr_v = new int[v3 / 4];
            Object[] arr_object = new Object[v3 / 4];
            System.arraycopy(this.j, 0, arr_v, 0, this.j.length);
            System.arraycopy(this.k, 0, arr_object, 0, this.k.length);
            this.j = arr_v;
            this.k = arr_object;
        }
        this.j[v2] = v;
        this.k[v2] = object0;
        this.l = v2 + 1;
    }

    public final l b() {
        try {
            l l0 = (l)super.clone();
            l0.j = (int[])this.j.clone();
            l0.k = (Object[])this.k.clone();
            return l0;
        }
        catch(CloneNotSupportedException cloneNotSupportedException0) {
            throw new AssertionError(cloneNotSupportedException0);
        }
    }

    public final void c() {
        int v = this.l;
        int[] arr_v = this.j;
        Object[] arr_object = this.k;
        int v2 = 0;
        for(int v1 = 0; v1 < v; ++v1) {
            Object object0 = arr_object[v1];
            if(object0 != l.m) {
                if(v1 != v2) {
                    arr_v[v2] = arr_v[v1];
                    arr_object[v2] = object0;
                    arr_object[v1] = null;
                }
                ++v2;
            }
        }
        this.i = false;
        this.l = v2;
    }

    @Override
    public final Object clone() {
        return this.b();
    }

    public final Object d(int v, Integer integer0) {
        int v1 = d.a(this.l, v, this.j);
        if(v1 >= 0) {
            Object object0 = this.k[v1];
            if(object0 != l.m) {
                return object0;
            }
        }
        return integer0;
    }

    public final void e(int v, Object object0) {
        int v1 = d.a(this.l, v, this.j);
        if(v1 >= 0) {
            this.k[v1] = object0;
            return;
        }
        int v2 = ~v1;
        int v3 = this.l;
        if(v2 < v3) {
            Object[] arr_object = this.k;
            if(arr_object[v2] == l.m) {
                this.j[v2] = v;
                arr_object[v2] = object0;
                return;
            }
        }
        if(this.i && v3 >= this.j.length) {
            this.c();
            v2 = ~d.a(this.l, v, this.j);
        }
        int v4 = this.l;
        if(v4 >= this.j.length) {
            int v5 = (v4 + 1) * 4;
            for(int v6 = 4; v6 < 0x20; ++v6) {
                int v7 = (1 << v6) - 12;
                if(v5 <= v7) {
                    v5 = v7;
                    break;
                }
            }
            int[] arr_v = new int[v5 / 4];
            Object[] arr_object1 = new Object[v5 / 4];
            System.arraycopy(this.j, 0, arr_v, 0, this.j.length);
            System.arraycopy(this.k, 0, arr_object1, 0, this.k.length);
            this.j = arr_v;
            this.k = arr_object1;
        }
        int v8 = this.l - v2;
        if(v8 != 0) {
            System.arraycopy(this.j, v2, this.j, v2 + 1, v8);
            System.arraycopy(this.k, v2, this.k, v2 + 1, this.l - v2);
        }
        this.j[v2] = v;
        this.k[v2] = object0;
        ++this.l;
    }

    public final int f() {
        if(this.i) {
            this.c();
        }
        return this.l;
    }

    public final Object g(int v) {
        if(this.i) {
            this.c();
        }
        return this.k[v];
    }

    @Override
    public final String toString() {
        if(this.f() <= 0) {
            return "{}";
        }
        StringBuilder stringBuilder0 = new StringBuilder(this.l * 28);
        stringBuilder0.append('{');
        for(int v = 0; v < this.l; ++v) {
            if(v > 0) {
                stringBuilder0.append(", ");
            }
            if(this.i) {
                this.c();
            }
            stringBuilder0.append(this.j[v]);
            stringBuilder0.append('=');
            Object object0 = this.g(v);
            if(object0 == this) {
                stringBuilder0.append("(this Map)");
            }
            else {
                stringBuilder0.append(object0);
            }
        }
        stringBuilder0.append('}');
        return stringBuilder0.toString();
    }
}

