...
public class GeofencingClient extends GoogleApi {
    ...
    public Task addGeofences(GeofencingRequest geofencingRequest0, PendingIntent pendingIntent0) {
        GeofencingRequest geofencingRequest1 = geofencingRequest0.zza(this.b);
        return this.doWrite(TaskApiCall.builder().run(new c(29, geofencingRequest1, pendingIntent0)).setMethodKey(0x978).build());
    }

    public Task removeGeofences(PendingIntent pendingIntent0) {
        return this.doWrite(TaskApiCall.builder().run(new e(1, pendingIntent0)).setMethodKey(0x979).build());
    }

    public Task removeGeofences(List list0) {
        return this.doWrite(TaskApiCall.builder().run(new s(list0)).setMethodKey(0x979).build());
    }
}
