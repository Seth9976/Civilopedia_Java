...
public final class zzan {
    public static void a(zzeu zzeu0, SQLiteDatabase sQLiteDatabase0, String s, String s1, String s2, String[] arr_s) {
        Cursor cursor0;
        if(zzeu0 == null) {
        ...
        label_10:
            zzeu0.zzk().zzc("Error querying for table", s, sQLiteException0);
            if(cursor0 != null) {
                goto label_12;
            }
            goto label_13;
    ...
    label_18:
        ...
        label_21:
            ...
            if(!hashSet0.isEmpty()) {
                zzeu0.zzk().zzc("Table has extra columns. table, columns", s, TextUtils.join(", ", hashSet0));
            }
            return;
        }
        catch(SQLiteException sQLiteException1) {
        }
        zzeu0.zzd().zzb("Failed to verify columns on table that was just created", s);
        throw sQLiteException1;
    }

    public static void b(zzeu zzeu0, SQLiteDatabase sQLiteDatabase0) {
        if(zzeu0 == null) {
            throw new IllegalArgumentException("Monitor must not be null");
        }
        File file0 = new File(sQLiteDatabase0.getPath());
        if(!file0.setReadable(false, false)) {
            zzeu0.zzk().zza("Failed to turn off database read permission");
        }
        if(!file0.setWritable(false, false)) {
            zzeu0.zzk().zza("Failed to turn off database write permission");
        }
        if(!file0.setReadable(true, true)) {
            zzeu0.zzk().zza("Failed to turn on database read permission for owner");
        }
        if(!file0.setWritable(true, true)) {
            zzeu0.zzk().zza("Failed to turn on database write permission for owner");
        }
    }
}
